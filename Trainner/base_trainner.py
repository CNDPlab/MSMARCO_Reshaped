...
class BaseTrainner(object):
    ...
    def init_trainner(self, resume_from=None, use_summary_writer=True, word_vocab=None, char_vocab=None):
        ...
        if resume_from is None:
            optimizer = t.optim.Adam([i for i in self.model.parameters() if i.requires_grad is True], lr=1e-3, weight_decay=0.0003)
            self.optim = ScheduledOptim(optimizer, 314, self.args.warm_up_step, self.global_step)
        else:
    ...
    def reserve_topk_model(self, k=5):
        all_saved_models = os.listdir(os.path.join(self.paths['exp_root'], 'saved_models'))
        k_best_models = self.get_best_k_model_path(os.path.join(self.paths['exp_root'], 'saved_models'), k)
        drops = [i for i in all_saved_models if i not in k_best_models]
        for i in drops:
            shutil.rmtree(os.path.join(self.paths['exp_root'],'saved_models',i))
